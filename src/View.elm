...
import Color
import Direction3d
import Html exposing (Html, button, div, h1, img, input, text, br, p)
import Html.Attributes exposing (style, src, alt, type_)
...
import Viewpoint3d
import Temperature
import Types exposing (buildText3d)
import String
import Html.Attributes exposing (list, step, id, value)
...
renderGame model =
    let
        ...
        he = Scene3d.blockWithShadow (Material.color Color.white) (Block3d.from (Point3d.meters 0 34 0) (Point3d.meters 2 36 2))
            ...
            case model.level of
                6 ->
                    let
                        rgb = 90
                        color =
                            if model.level6Lock then Color.black
                            else
                                case model.gameStatus of
                                    Play -> Color.rgb255 rgb rgb rgb
                                    Interlude time duration _ _ _ ->
                                        let t = time / duration * rgb |> round
                                        in Color.rgb255 t t t
                                    _ -> Color.rgb255 rgb rgb rgb
                    in Scene3d.block (Material.matte color) floor
                _ -> Scene3d.block (Material.matte Color.darkGrey) floor
                    ...
                    let
                        ...
                        color =
                            if model.level6Lock then Color.black
                                ...
                                case model.gameStatus of
                                    Play -> Color.rgb255 r g b
                                    Interlude time duration _ _ _ ->
                                        Color.rgb255 (time / duration * r |> round) (time / duration * g |> round) (time / duration * b |> round)
                                    _ -> Color.rgb255 r g b
                    in Scene3d.backgroundColor color
                _ -> Scene3d.backgroundColor model.scene.background
        ...
        texts3d =
            if model.godMode then
                if model.world == Normal then List.foldl godText [] model.texts3d
                else List.foldl godText [] model.texts3dRev
            else
                if model.world == Normal then
        ...
        pickText text3d list =
            if text3d.time <= 0 then list
            else text3d.entities :: list

        godText text3d list = list ++ [text3d.entities]
        

        entities = --这么改很容易忘改回来，不需要settings的话在init settings = Scene3d.nothing
            if model.level == 6 then [ ground, self, settings, he] ++ actives ++ nactives ++ wall ++ cars ++ texts3d
            else  [ ground, self, settings] ++ actives ++ nactives ++ wall ++ cars ++ texts3d
        ...
        camera =
            case model.level of
                ...
                _ ->
                    Camera3d.perspective
                        --透视图
                        { viewpoint = Viewpoint3d.orbitZ model.camera --调用摄像机参数，垂直视域固定。
                        , verticalFieldOfView = Angle.degrees 30
                        }

        lights = switchLight model
        opacity =
            case model.gameStatus of    
                Interlude time duration _ _ _ ->
                    if model.level == 4 && time >= 700 then "0"
                    else "1"
                Animation frame ->
                    if model.level == 6 && frame == 5 && model.frameTime > 1 then
                        String.fromFloat ((toFloat model.frameTime) / 80)
                    else if model.level == 6 && frame > 5 then "0"
                    else "1"
                _ -> "1"
    in
        -- if isShow then
        div [ style "opacity" opacity
            ...
            [
            Scene3d.custom -- 第一个调整render质量
                { antialiasing = Scene3d.supersampling <| 0.1*(toFloat model.scene.renderOpt) -- 1.5调整渲染倍率，决定画质，=sqrt(实际渲染分辨率/窗口分辨率)
                , entities = entities
                , camera = camera
        ...
        -- else div[style "background-color" "black"][]
...
renderText model =
    let
        ...
        top = String.fromFloat model.text.top ++ "%"
        left = String.fromFloat model.text.left ++ "%"
        size = String.fromFloat model.text.size ++ "vw"
        opacity = String.fromFloat model.text.opacity
    in
...
renderEscMenu model = 
    ...
    let
        renderOpt = model.scene.renderOpt
    in
    div [ style "background-color" "#515158" 
        , style "width" "40%"
        , style "height" "40%"
        , style "position" "absolute"
        , style "top" "30%"
        ...
        this =
            case model.actives of
                ...
                [] ->
                    model.self
        opacity =
            if model.level == 0 then toFloat model.frameTime / 100 
            else
                case model.gameStatus of
                    LoseFadeOut time duration -> time / duration
                    WinFadeOut _ time duration -> time / duration
                    StartFadeIn time duration -> time / duration
                    LevelChange _ -> 0
                    Animation frame ->
                        if frame == 5 && model.level == 1 then 0 else 1
                    _ -> 1
    in
    div
        ...
        [
            div
                ...
                , renderText model
                , renderEscMenu model
                -- , Html.p [] [ Html.text <| Debug.toString <| ifNextStepAvailable model.self R model ]
                -- , Html.p [] [ Html.text <| Debug.toString model.world ]
                , preGameView model
                , p --debug time
                    [ style "position" "absolute"
                    , style "top" "1%"
...
switchLight model =
    case (model.level, model.world) of
        ...
        ( 2, Normal ) ->
            let
                light1Lumen = toFloat (60000 - 40 * model.time)
                softLightingLumen = 80 - 0.05 * (toFloat model.time)
                light1 =
        ...
        ( 4, Normal) -> 
            let
                ratio = 0.25
                center = Point3d.toRecord Length.inMeters (Block3d.centerPoint model.self.this)
                light1 =
                    case model.gameStatus of
                        Interlude time duration _ _ _ ->
        ...
        ( 6, Normal) -> 
            let
                ...
                light3 =
                    Light.point (Light.castsShadows True)
                        { position = Point3d.meters 0 38 3
                        -- , chromaticity = Light.chromaticity { x = 0.345669, y = 0.358496 }
                        , chromaticity = Light.incandescent
                ...
                --         { position = Point3d.meters 0 0 20
                --         , chromaticity = Light.incandescent
                --         , intensity = LuminousFlux.lumens (100000 * lumi / max)
                --         }

                max = 80
                lumi =
                    if model.level6Lock then 1.5
                    else
                        case model.gameStatus of
                            Play -> max
                            Interlude time duration _ _ _ -> time / duration * max
                            _ -> max
                softLighting =
                    Light.soft
                        { upDirection = Direction3d.positiveZ
            ...
            in
                if model.level6Lock then Scene3d.threeLights light1 light2 softLighting
                else Scene3d.fourLights light3 light4 light5 softLighting
