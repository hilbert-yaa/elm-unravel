...
actives =
    let
        p1 = Point3d.meters -13 -13 1
        p2 = Point3d.meters -7 -15 1
        p3 = Point3d.meters -1 -15 1
        p4 = Point3d.meters 5 -13 1
        p5 = Point3d.meters 19 -7 1
        p6 = Point3d.meters 19 -3 1
        p7 = Point3d.meters 5 15 1
        p8 = Point3d.meters 15 5 1
        p9 = Point3d.meters -3 19 1
        p10 = Point3d.meters -5 19 1
        p11 = Point3d.meters -7 19 1
        p12 = Point3d.meters -9 19 1
        p13 = Point3d.meters -11 19 1
        p14 = Point3d.meters -3 15 1
        p15 = Point3d.meters -5 15 1
        p16 = Point3d.meters -7 15 1
        p17 = Point3d.meters -9 15 1
        p18 = Point3d.meters -11 15 1
        blocks = List.map (\center -> buildColorBlock center Color.lightGray Nothing Up) [p1,p2,p3,p4,p5,p6,p7,p8,p9,p10,p11,p12,p13,p14,p15,p16,p17,p18]
    in
    blocks
...
settings =
    let 
        lamps = [lamp (Point3d.meters 20 20 0) 225, lamp (Point3d.meters 20 -20 0) 135, lamp (Point3d.meters -20 20 0) 315
                , lamp (Point3d.meters -20 -20 0) 45, lamp (Point3d.meters -6 -14 0) 270, lamp (Point3d.meters 10 -12 0) 270
                , lamp (Point3d.meters 16 0 0) 0, lamp (Point3d.meters 0 10 0) 90]
    in 
    Scene3d.group
        <| lamps
...
init =
    -- 方块的边长是2
    let
        initCenter = Point3d.meters -19 -19 1 --self初始位置
        initBlock = buildBlock initCenter
        -- initGoal = Point3d.meters 1 19 1
        initGoal = Point3d.meters -19 19 1
        groundSize = GroundSize 50 50 --地面边界
        mapSize = GroundSize 10 10 --地图边界（空气墙）
...
buildStore center = 
    ...
    in
        let
            blocks = List.map (\block -> Scene3d.blockWithShadow (Material.color Color.blue) block) [block1]
        in
